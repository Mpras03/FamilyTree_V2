...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-xl font-semibold">Edit Anggota Keluarga</h2>
        <a href="{{ route('admin.persons.index') }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; Kembali</a>
    </div>

    <form method="POST" action="{{ route('admin.persons.update', $person) }}" enctype="multipart/form-data" class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
